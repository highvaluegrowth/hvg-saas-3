...
export async function POST(request: NextRequest) {
    try {
        ...
        // Operator Route requires valid staff/admin role
        ...
        const systemInstruction = buildOperatorSystemPrompt(
        ...
        const response = await genai.models.generateContent({
            model: GEMINI_MODEL,
            contents,
            config: { systemInstruction, tools: operatorTierTools },
        });
        ...
        batch.set(convRef, { userId: uid, persona: 'operator', tier: 'operator', tenantId, updatedAt: new Date() }, { merge: true });
