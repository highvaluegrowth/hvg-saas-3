...
export async function executeOperatorTool(
    ...
    if (toolName === 'draft_social_post') {
        ...
        // Suppress unused variable warnings — fetched for potential future use
        void tone;
        void platform;
        const tenantSnap = await adminDb.collection('tenants').doc(tenantId).get();
        const tenant = tenantSnap.data() ?? {};
        const houseName: string = (tenant.name as string) ?? 'our sober living home';
        void houseName;
        return {
            message: `To draft a ${postType} post, use the AI Compose tool at /${tenantId}/marketing/compose. Here is a quick draft based on your context: "${context}". Navigate to Marketing → Compose for the full AI generation experience with hashtag selection.`,
    ...
    if (toolName === 'get_marketing_posts') {
        ...
        const posts = snap.docs.map(d => {
            const p = d.data();
            return { id: p.id, content: (p.content as string)?.slice(0, 80), status: p.status, platforms: p.platforms, scheduledAt: p.scheduledAt, createdAt: p.createdAt };
        });
