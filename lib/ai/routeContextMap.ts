...
export function getRouteContext(pathname: string): string {
    ...
    const exact: Record<string, string> = {
        ...
        '/chores': 'The user is on the Chores board. Pending chore assignments are relevant.',
        '/incidents': 'The user is on the Incident Log. Recent incident reports may be relevant.',
        '/rides': 'The user is on the Transportation page. Active and pending ride requests are relevant.',
    ...
    if (normalized.startsWith('/lms/') && normalized.includes('/builder')) {
        return 'The user is in the Course Builder, actively editing lesson content. Help with content creation, quiz design, or curriculum structure.';
    }
