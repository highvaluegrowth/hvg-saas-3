...
export function CommandCard({ type, data }: CommandCardProps) {
    ...
    switch (type) {
        ...
        case 'create_event':
            return (
                <div className="p-3 bg-amber-50 border border-amber-200 text-amber-800 rounded-xl text-sm flex items-center gap-2">
                        ...
                        <ul className="space-y-2">
                            {((d.requests as any[]) ?? []).map((req, i) => (
                                <li key={i} className="bg-white border text-xs border-stone-100 p-2 rounded-lg flex justify-between items-center">
                                    ...
                                    <span className="font-medium text-amber-600">{new Date(req.time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                                </li>
                            ))}
                        </ul>
                    )}
                            ...
                            {((d.requests as any[]) ?? []).map((req, i) => (
                                <li key={i} className="bg-white border text-xs border-stone-100 p-2 rounded-lg flex flex-col gap-1">
                                    <div className="flex justify-between font-medium">
                                        <span>{req.name}</span>
                                        <span className="text-amber-600 capitalize">{req.status}</span>
                                    </div>
